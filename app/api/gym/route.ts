...
export async function GET(request:Request){try{const user=await getAppUser(request);if(!user)return response({user:null,crews:[],crew:null});const result=response(await snapshot(db(),user,new URL(request.url).searchParams.get('crew')));const token=readCookie(request,cookieName('session',request.url));if(token&&await renewGoogleSession(db(),token))result.headers.append('Set-Cookie',authCookie('session',token,request.url,SESSION_SECONDS));return result;}catch(e){return failure(e);}}
