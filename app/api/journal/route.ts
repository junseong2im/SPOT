...
async function handle(request:Request,write=false){try{
 ...
 if(!write){const params=new URL(request.url).searchParams,isDashboard=params.get('dashboard')==='1';const result=Response.json(isDashboard?await healthDashboard(env.DB,user.userId):await journalPage(env.DB,user.userId,params.get('cursor'),params.get('kind')),{headers:{'Cache-Control':'no-store'}});if(isDashboard){const token=readCookie(request,cookieName('session',request.url));if(token&&await renewGoogleSession(env.DB,token))result.headers.append('Set-Cookie',authCookie('session',token,request.url,SESSION_SECONDS));}return result;}
