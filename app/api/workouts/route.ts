...
async function handle(request:Request,write=false){try{
 ...
 if(!write){const params=new URL(request.url).searchParams;return reply(params.get('history')==='1'?await workoutHistory(env.DB,user.userId,params.get('cursor')):await trainingSnapshot(env.DB,user.userId,params.get('id')));}
