...
import {trainingAction,trainingSnapshot,workoutHistory,nextWorkoutPlan} from '@/lib/training-service';
...
async function handle(request:Request,write=false){try{
 ...
 if(!write){const params=new URL(request.url).searchParams;if(params.has('plan'))return reply(await nextWorkoutPlan(env.DB,user.userId,params.get('plan')!));return reply(params.get('history')==='1'?await workoutHistory(env.DB,user.userId,params.get('cursor'),params.get('q')??''):await trainingSnapshot(env.DB,user.userId,params.get('id')));}
