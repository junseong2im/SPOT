 ...
 return db.transaction(async tx=>{
  ...
  if(input.action==='start'){
   ...
   const state:WorkoutState={restSeconds:90,note:'',discomfort:'unreported',conditionsConfirmed:false,exercises:routine.exercises.map(e=>{
    ...
    return {id:e.id,name:e.name,timed,plannedReps,convention,equipmentLabel:prior?.equipmentLabel??'',prescription:e.prescription??'',sets:Array.from({length:e.sets},(_,i)=>({id:crypto.randomUUID(),kind:'working' as const,weight:priorSets[i]?.weight??null,reps:timed?null:priorSets[i]?.reps??e.reps,seconds:timed?priorSets[i]?.seconds??e.durationSeconds!:null,rir:null,done:false}))};
   })};
