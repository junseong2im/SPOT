...
const identity=(e:ExerciseLog)=>[normalizeExerciseSearch(e.name),e.convention,e.equipmentLabel.trim().toLowerCase(),e.timed].join('|');
export function analyzeTraining(history:Workout[],profile:TrainingProfile|null,now=Date.now()){
 ...
 const sessionsByExercise=new Map<string,{workout:Workout;exercise:ExerciseLog}[]>();
 for(const w of completed){const seen=new Set<string>();for(const e of w.state.exercises){const key=identity(e);if(seen.has(key)||!working(e).length)continue;seen.add(key);const entries=sessionsByExercise.get(key)??[];entries.push({workout:w,exercise:e});sessionsByExercise.set(key,entries);}}
 ...
 for(const entries of sessionsByExercise.values()){
  ...
  if(e.timed){advice.push({exercise:e.name,status:'maintain',title:'유지 시간 추세를 확인하세요',reasons:[`최근 기록: ${working(e).reduce((n,s)=>n+(s.seconds??0),0)}초`,`시간 기준 운동에는 중량·횟수 증가 규칙을 적용하지 않습니다.`],missing});continue;}
 ...
 return {windowDays:90,sessions:completed.length,trainingDays:new Set(completed.map(w=>localDate(new Date(w.finished_at!)))).size,last7Days:new Set(completed.filter(w=>w.finished_at!>=now-7*86400000).map(w=>localDate(new Date(w.finished_at!)))).size,weeklyTarget:profile?.daysPerWeek??null,sets:completed.reduce((n,w)=>n+workoutTotals(w).sets,0),missingWeights:completed.reduce((n,w)=>n+workoutTotals(w).missingWeights,0),weekly,advice:advice.slice(0,30),sources:trainingSources};
}
