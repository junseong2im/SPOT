...
type NotificationRow={id:string;user_id:string;crew_id:string;session_id:string;title:string;body:string;expires_at:number|string};
export async function dispatchPush(db:Database,options:{enabled:boolean;send:Sender;now?:number}) {
 ...
 for(const item of due){
  ...
  const notification=await db.prepare("UPDATE notifications SET push_state='sending',lease_until=? WHERE id=? AND push_state='queued' RETURNING id,user_id,crew_id,session_id,title,body,expires_at").bind(now+120000,item.id).first<NotificationRow>();
  ...
  for(const subscription of subscriptions){
   ...
   try{
    await options.send({endpoint:subscription.endpoint,keys:{p256dh:subscription.p256dh,auth:subscription.auth}},JSON.stringify({id:notification.id,title:notification.title,body:notification.body,url:`/?crew=${encodeURIComponent(notification.crew_id)}&session=${encodeURIComponent(notification.session_id)}`}));accepted++;
   }catch(error){
