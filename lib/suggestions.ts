...
export async function listSuggestions(db:Database,userId:string){const admin=await musicAdmin(db,userId);const items=(await db.prepare('SELECT id,user_id,author_name,category,title,body,link,status,reply,revision,created_at FROM suggestions'+(admin?'':' WHERE user_id=?')+' ORDER BY created_at DESC,id LIMIT 100').bind(...(admin?[]:[userId])).all<Suggestion>()).results;return {admin,items};}
