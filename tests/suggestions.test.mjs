...
test('suggestions are private, idempotent and only administrators can reply or change status',async()=>{
 ...
 try{
  ...
  assert.equal((await listSuggestions(db,'friend')).items.length,1);
  ...
  const saved=(await listSuggestions(db,'friend')).items[0];assert.equal(saved.status,'planned');assert.equal(saved.reply,update.reply);
