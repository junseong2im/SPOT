...
export function postgresSql(sql: string) {
  let parameter = 0;
  return sql.replace(/'(?:''|[^'])*'|"(?:""|[^"])*"|\?|\b(?:crews|members|personal|oauth_transactions|auth_sessions|notifications|notification_preferences|push_subscriptions|app_admins|music_recommendations)\b/g, token => {
